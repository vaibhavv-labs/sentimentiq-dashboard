...
/* 🚨 DON'T HIDE SIDEBAR TOGGLE */
#MainMenu, footer { visibility: hidden; }
header { background: transparent !important; }

/* ✅ STYLE SIDEBAR SAFELY */
...
.pill {
    ...
    background: rgba(5,240,160,0.08);
    border: 1px solid rgba(5,240,160,0.25);
    color: var(--green);
...
.mcard-val {
    font-family: 'DM Mono', monospace;
    font-size: 2.3rem; font-weight: 500; color: var(--text);
...
.cbox {
    background: var(--bg2); border: 1px solid var(--border);
    border-radius: 16px; padding: 22px 22px 14px; margin-bottom: 16px;
}
.cbox-label {
...
.err-wrap {
    ...
    border: 1px solid rgba(255,63,91,0.18);
    border-radius: 14px; margin-top: 18px;
    animation: fadeup .4s ease;
}
...
.stRadio label {
    ...
    border-radius: 10px !important; padding: 10px 16px !important; cursor: pointer !important;
}
...
.info-card {
    background: #09121f; border: 1px solid var(--border);
...
def predict(text):
    ...
    return "Negative", conf, "☹️", COLOR["red"],    "rgba(255,63,91,0.08)",  "rgba(255,63,91,0.28)"
...
def sentiment_charts(pos, neu, neg, tot):
    ...
    with c1:
        ...
        fig = go.Figure(go.Pie(
            ...
            hole=0.60,
            marker=dict(colors=["#05f0a0","#ffc542","#ff3f5b"],
...
with st.sidebar:
    ...
    <div style='padding:4px 0 18px;'>
        <div style='font-family:Syne,sans-serif;font-size:1.2rem;font-weight:800;
                    color:#e8edf5;letter-spacing:-0.5px;'>🧠 SentimentIQ</div>
    ...
    <div style='font-size:0.68rem;font-weight:600;text-transform:uppercase;
                letter-spacing:1.2px;color:#4a5e7a;margin-bottom:12px;'>Model Info</div>
    <div class='info-card'>
        ...
        if "text" not in df.columns:
            ...
            if tot > 0:
                if pos > neg and pos > neu:
                    dominant = "Positive"
                    dom_icon = "😊"
                    dom_color = "#05f0a0,#00a86b"
                elif neg > pos and neg > neu:
                    dominant = "Negative"
                    dom_icon = "😡"
                    dom_color = "#ff3f5b,#cc0020"
                elif neu > pos and neu > neg:
                    dominant = "Neutral"
                    dom_icon = "😐"
                    dom_color = "#ffc542,#e6a800"
                else:
                    dominant = "Mixed"
                    dom_icon = "⚖️"
                    dom_color = "#3d9eff,#0066cc"
            else:
                dominant = "N/A"
                dom_icon = "➖"
                dom_color = "#4a5e7a,#2d3748"

            st.markdown("<div class='metric-grid'>", unsafe_allow_html=True)
            for lbl, val, sub, grad, icon in [
                ("Total Records",  f"{tot:,}",  f"{len(df)-tot} skipped",        "#3d9eff,#0066cc", "📝"),
                ("% Positive",     f"{pos/tot*100:.1f}%" if tot else "0.0%", f"{pos:,} texts", "#05f0a0,#00a86b", "😊"),
                ("% Negative",     f"{neg/tot*100:.1f}%" if tot else "0.0%", f"{neg:,} texts", "#ff3f5b,#cc0020", "😡"),
...
# ═══════════════════════════════════════════════
    ...
    yt_url = st.text_input("YouTube URL", placeholder="https://www.youtube.com/watch?v=xxxxxxx",
    ...
    if st.button("▶️  Fetch & Analyze Comments"):
        if not yt_url.strip():
            ...
            if not match:
                st.error("❌  Invalid YouTube URL. Example: https://www.youtube.com/watch?v=dQw4w9WgXcQ")
            else:
                ...
                try:
                    ...
                    st.error(f"❌  Error: {e}")
                    st.info("Make sure your YOUTUBE_API_KEY is correct in .streamlit/secrets.toml")
...
@st.cache_resource
def load_model():
    mdl = AutoModelForSequenceClassification.from_pretrained("final_model")
...
# ═══════════════════════════════════════════════
    ...
    st.markdown("</div>", unsafe_allow_html=True)
>>>>>>> d68145623c123ccc1715dc3aaa5a5dfae054eb26
